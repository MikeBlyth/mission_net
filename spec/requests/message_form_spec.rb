...
if ENV['DRB']
  puts "**** Skipping message_form_spec.rb as it cannot be run under DRB (Spork) ****"
else
  describe 'Message form', :no_spork => true do
    # Need to override delivery of message because it's on DelayJob and crashes tests
    # Restoring :old_deliver_message does not work in Spork, so this test should
    # Not be run under Spork, except perhaps by itself
    before(:all) do
